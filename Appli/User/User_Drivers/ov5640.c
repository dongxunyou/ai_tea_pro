...
/**
 * @brief   读OV5640寄存�? * @param   reg: 寄存器地址
 * @retval  寄存器�? */
...
/**
 * @brief   写OV5640寄存�? * @param   reg: 寄存器地址
 * @param   data: 寄存器�? * @retval  �? */
...
/**
 * @brief   控制OV5640闪光�? * @param   sw: 闪光灯状�? * @param   0: 关闭
 * @param   1: 开�? * @retval  �? */
...
/**
 * @brief   设置OV5640色彩饱和�? * @param   level: 色彩饱和度等�? * @arg     0~6: -3~+3
 * @retval  �? */
...
/**
 * @brief   设置OV5640对比�? * @param   level: 对比度等�? * @arg     0~6: -3~+3
 * @retval  �? */
...
uint8_t ov5640_outsize_set(uint16_t offx, uint16_t offy, uint16_t width, uint16_t height)
{
    ...
    /* 输出尺寸在ISP上面的取图范�?     * 范围: xsize-2*offx,ysize-2*offy
     */
...
/**
 * @brief   设置OV5640图像开窗大�? * @param   offx: 图像在传感器的X方向偏移
